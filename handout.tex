...
and is explained in more detail in \cref{sipfcake}.
...
Note the length of `cMap.R' is exactly 100 lines. The additional length is due
...
`data cube'. If this is confusing, just remember that
the `square' weight matrix --- where each column represents a zone
and each row represents an individual --- can be returned to by
specifying the third dimension. Because dimensions in R are specified by
the square brackets, the initial weights can be called by typing
...
set of command repeatedly. \emph{i in for(1:9) {print i}}, for example will print

...
For each constraint (age or sex for example), the current weight is multiplied
by the respective total from the geographically aggregated table (Table~\ref{t:s}) and divided by the
corresponding marginal total of the survey data (see \cref{t:m}). This is done one constraint at a time, as described in \cref{eq:ipf-word} and more precisely by \cref{eq:ipf} for constraint $i$ (age in this case):
...
After creating $Weight_{New}$, the individual level data is recreated before the next weight can be calculated using the next constraint. % To do this you...
...
sum of each weight ($W_2$, $W_3$, etc.) is always equal to the total population, which is a useful check.
...
The next stage is to re-aggregate the individual-level data,
to compare the marginal totals with the constraint tables
ready for the second constraint --- this will result in
